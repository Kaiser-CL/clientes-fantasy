...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($pdo)) {
    ...
    if (isset($_POST['accion_evento']) && $_POST['accion_evento'] === 'marcar_finalizado') {
        try {
            ...
            if ($id_evento_final > 0) {
                $stmtOld = $pdo->prepare("SELECT * FROM eventos WHERE id_evento = ?");
                $stmtOld->execute([$id_evento_final]);
                $datosEvento = $stmtOld->fetch(PDO::FETCH_ASSOC);

                $stmt_upd = $pdo->prepare("UPDATE eventos SET estado = 'finalizado' WHERE id_evento = ?");
                $stmt_upd->execute([$id_evento_final]);

                if (function_exists('registrarBitacora')) {
                    registrarBitacora($pdo, 'ACTUALIZAR', 'eventos', $id_evento_final, $datosEvento, ['estado' => 'finalizado']);
                }

                $mensaje = "El Evento #" . $id_evento_final . " fue marcado como FINALIZADO.";
            }
        } catch (Exception $e) {
            $error_db = "Error al marcar evento como finalizado: " . $e->getMessage();
        }
    }
...
if (isset($pdo)) {
    try {
        ...
        $sql_eventos = "SELECT 
                ...
                WHERE LOWER(COALESCE(e.estado, 'confirmado')) NOT IN ('cancelado','inactivo')
...
<!-- FORMULARIO OCULTO PARA MARCAR EVENTO COMO FINALIZADO -->
<form action="" method="POST" id="form_finalizar_evento_directo">
    <input type="hidden" name="accion_evento" value="marcar_finalizado">
    <input type="hidden" name="id_evento" id="id_evento_finalizar_input">
</form>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="nuevo_id_servicio" id="modal_select_nuevo_servicio" class="form-select border-success" onchange="evaluarSelectNuevoExtra()">
                                ...
                                <?php foreach ($extras_catalogo as $serv): 
                                    ...
                                    $ubi_extra = strtolower($serv['ubicacion'] ?? 'ambos');
                                ?>
                                    <option value="<?= $serv['id_servicio'] ?>" data-precio="<?= $serv['precio_servicio'] ?>" data-tipo-cobro="<?= $es_p ?>" data-ubicacion="<?= $ubi_extra ?>">
...
function abrirModalDesglose(evt) {
    ...
    let selectExtraModal = document.getElementById('modal_select_nuevo_servicio');
    if (selectExtraModal) {
        Array.from(selectExtraModal.options).forEach(opt => {
            let ubiExtra = opt.getAttribute('data-ubicacion') || 'ambos';
            if (opt.value === '') return;

            if (ubiExtra === 'ambos' || (esJardin && ubiExtra === 'jardin') || (!esJardin && ubiExtra === 'carmelo')) {
                opt.style.display = 'block';
                opt.disabled = false;
            } else {
                opt.style.display = 'none';
        ...
        if (selectExtraModal.value && Array.from(selectExtraModal.options).every(opt => opt.value !== selectExtraModal.value || opt.style.display === 'none')) {
            selectExtraModal.value = '';
...
function confirmarFinalizarDirecto(idEvento, nombreEvento) {
    let folio = "#EV-" + String(idEvento).padStart(5, '0');
    if (confirm(`¿Deseas marcar como FINALIZADO el evento ${folio} ("${nombreEvento}")? Esta acción indicará que el evento ya concluyó.`)) {
        document.getElementById('id_evento_finalizar_input').value = idEvento;
        document.getElementById('form_finalizar_evento_directo').submit();
    }
}
</script>
